...
func main() {
	...
	pipe1 := pipeline.NewStep("step1", llm1, prompt1, decoder.NewDefaultDecoder(), nil)
	...
	pipe2 := pipeline.NewStep("step2", llm2, prompt2, decoder.NewJSONDecoder(myout), nil)
	...
	pipe3 := pipeline.NewStep("step3", llm1, prompt3, decoder.NewRegExDecoder(`(\w+?)\s(\w+?)\s(.*)`), nil)
